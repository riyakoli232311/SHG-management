...
// Helper to get savings by member
export const getSavingsByMember = (memberId: string): Saving[] => {
  return savings.filter(s => s.member_id === memberId);
};

// Helper to get total savings
export const getTotalSavings = (): number => {
  return savings.reduce((sum, s) => sum + s.amount, 0);
};

// Helper to get member total savings
export const getMemberTotalSavings = (memberId: string): number => {
