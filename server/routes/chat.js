...
// ── Tools ─────────────────────────────────────────────────────
const TOOLS = [
  { type: 'function', function: { name: 'get_dashboard', description: 'Get overall SHG statistics: total members, total savings, active loans, pending/overdue repayments, pending loan applications count.', parameters: { type: 'object', properties: {}, required: [] } } },
  ...
  { type: 'function', function: { name: 'get_member', description: 'Get full profile of a member by ID including their savings and loans.', parameters: { type: 'object', properties: { member_id: { type: 'integer' } }, required: ['member_id'] } } },
  { type: 'function', function: { name: 'add_member', description: 'Add a new member to the SHG.', parameters: { type: 'object', properties: { name: { type: 'string' }, phone: { type: 'string' }, age: { type: 'integer' }, village: { type: 'string' }, gram_panchayat: { type: 'string' }, block: { type: 'string' }, district: { type: 'string' }, state: { type: 'string' }, pin_code: { type: 'string' }, role: { type: 'string', enum: ['member', 'president', 'secretary', 'treasurer'] }, occupation: { type: 'string' }, husband_name: { type: 'string' }, joined_date: { type: 'string' }, caste_category: { type: 'string', enum: ['general', 'obc', 'sc', 'st'] }, bpl_status: { type: 'boolean' }, income: { type: 'number' }, aadhar: { type: 'string' }, bank_account: { type: 'string' }, bank_ifsc: { type: 'string' } }, required: ['name'] } } },
  { type: 'function', function: { name: 'update_member', description: 'Update an existing member by ID. Pass only the fields to change.', parameters: { type: 'object', properties: { member_id: { type: 'integer' }, name: { type: 'string' }, phone: { type: 'string' }, age: { type: 'integer' }, role: { type: 'string', enum: ['member', 'president', 'secretary', 'treasurer'] }, status: { type: 'string', enum: ['active', 'inactive'] }, occupation: { type: 'string' }, income: { type: 'number' }, village: { type: 'string' }, gram_panchayat: { type: 'string' }, block: { type: 'string' }, district: { type: 'string' }, state: { type: 'string' }, pin_code: { type: 'string' }, bank_account: { type: 'string' }, bank_ifsc: { type: 'string' }, aadhar: { type: 'string' }, husband_name: { type: 'string' }, caste_category: { type: 'string', enum: ['general', 'obc', 'sc', 'st'] }, bpl_status: { type: 'boolean' } }, required: ['member_id'] } } },
  ...
  { type: 'function', function: { name: 'get_savings_defaulters', description: 'List members who have NOT paid savings for a given month and year. Use this when asked "who hasn\'t paid this month?" or "savings defaulters".', parameters: { type: 'object', properties: { month: { type: 'integer', description: 'Month number 1-12' }, year: { type: 'integer', description: 'Four-digit year e.g. 2026' } }, required: ['month', 'year'] } } },
  ...
  { type: 'function', function: { name: 'get_loans', description: 'Fetch disbursed loans. Filter by member_id or status.', parameters: { type: 'object', properties: { member_id: { type: 'integer' }, status: { type: 'string', enum: ['active', 'closed'] } }, required: [] } } },
  { type: 'function', function: { name: 'get_loan_applications', description: 'Fetch loan applications submitted by members. Filter by status or member_id. Use this for "who applied for a loan?", "pending loan requests", "loan application status".', parameters: { type: 'object', properties: { status: { type: 'string', enum: ['pending', 'approved', 'rejected'] }, member_id: { type: 'integer' } }, required: [] } } },
  ...
  { type: 'function', function: { name: 'get_meetings', description: 'List all meetings with attendance summary (present/absent counts).', parameters: { type: 'object', properties: {}, required: [] } } },
  { type: 'function', function: { name: 'get_meeting_attendance', description: 'Get attendance details for a specific meeting (who attended, who was absent, fines). Also use to find a member\'s attendance history. Use this for "who missed last meeting?", "attendance report", "meeting fines".', parameters: { type: 'object', properties: { meeting_id: { type: 'integer', description: 'Specific meeting ID. Use get_meetings first to find the ID.' }, member_id: { type: 'integer', description: 'Get attendance history for a specific member across all meetings.' } }, required: [] } } },
