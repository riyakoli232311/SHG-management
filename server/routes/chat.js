...
const TOOLS = [
  {
    ...
    function: {
      name: 'get_dashboard',
      description: 'Get overall SHG statistics: total members, total savings, active loans count, pending/overdue repayments.',
  ...
  },
  {
    ...
    function: {
      name: 'get_savings',
      description: 'Fetch savings records. Filter by member_id, month+year, or get all.',
      parameters: {
        type: 'object',
        properties: {
          member_id: { type: 'integer' },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          year: { type: 'integer' },
          amount: { type: 'number' },
          payment_mode: { type: 'string', enum: ['cash', 'upi', 'bank_transfer'] },
  ...
  },
  {
    ...
    function: {
      name: 'create_loan',
      description: 'Disburse a new loan to a member. Auto-generates EMI repayment schedule.',
      parameters: {
        type: 'object',
        properties: {
          member_id: { type: 'integer' },
          loan_amount: { type: 'number' },
          interest_rate: { type: 'number', description: 'Annual rate %, default 2' },
          tenure_months: { type: 'integer', description: 'Default 12' },
          purpose: { type: 'string' },
          disbursed_date: { type: 'string', description: 'YYYY-MM-DD' },
        },
  ...
  },
  {
    ...
    function: {
      name: 'get_repayments',
      description: 'Get EMI repayment records. Filter by loan_id, member_id, or status.',
      parameters: {
        type: 'object',
        properties: {
          loan_id: { type: 'integer' },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Mark an EMI as paid. Auto-closes the loan if all EMIs are paid.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'get_meetings',
      description: 'List all meetings with attendance and details.',
      parameters: { type: 'object', properties: {}, required: [] },
    },
  },
  {
...
async function callGroq(payload, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (response.status === 429) {
      ...
      // Extract wait time from Groq message e.g. "Please try again in 4.595s"
...
// Only keeps the last N user+assistant pairs so we stay under the limit.
function trimMessages(messages, maxPairs = 4) {
  const system  = messages.filter(m => m.role === 'system');
  const convo   = messages.filter(m => m.role !== 'system');
  const trimmed = convo.slice(-(maxPairs * 2));
...
async function executeTool(name, args, shgId, sql) {
  ...
  switch (name) {
    ...
    case 'get_member': {
      const memberId = toInt(args.member_id);
      const [member] = await sql`SELECT * FROM members WHERE id = ${memberId} AND shg_id = ${shgId}`;
      if (!member) return { error: 'Member not found' };
      const savings = await sql`SELECT * FROM savings WHERE member_id = ${memberId} ORDER BY year DESC, month DESC`;
      const loans   = await sql`SELECT * FROM loans WHERE member_id = ${memberId} AND shg_id = ${shgId} ORDER BY disbursed_date DESC`;
    ...
    case 'add_member': {
      const [member] = await sql`
        ...
        VALUES (
          ${shgId}, ${args.name}, ${args.phone || null}, ${toInt(args.age)},
          ${toNum(args.income)}, ${args.aadhar || null},
    ...
    case 'update_member': {
      ...
      const [member] = await sql`
        UPDATE members SET
          ...
          age          = COALESCE(${toInt(args.age)},    age),
          role         = COALESCE(${args.role},          role),
          status       = COALESCE(${args.status},        status),
          occupation   = COALESCE(${args.occupation},    occupation),
          income       = COALESCE(${toNum(args.income)}, income),
          village      = COALESCE(${args.village},       village),
          bank_account = COALESCE(${args.bank_account},  bank_account),
          bank_ifsc    = COALESCE(${args.bank_ifsc},     bank_ifsc),
          updated_at   = now()
        WHERE id = ${memberId} AND shg_id = ${shgId}
        RETURNING *
      `;
    ...
    case 'get_savings': {
      ...
      if (args.member_id) {
        ...
        rows = await sql`
          ...
          WHERE s.shg_id = ${shgId} AND s.member_id = ${memberId}
        ...
        rows = await sql`
          SELECT s.*, m.name AS member_name FROM savings s
          JOIN members m ON m.id = s.member_id
          WHERE s.shg_id = ${shgId} AND s.month = ${month} AND s.year = ${year}
          ORDER BY m.name`;
      } else {
        rows = await sql`
          SELECT s.*, m.name AS member_name FROM savings s
    ...
    case 'record_saving': {
      ...
      const [member] = await sql`SELECT id, name FROM members WHERE id = ${memberId} AND shg_id = ${shgId}`;
      if (!member) return { error: `Member with ID ${memberId} not found in this SHG` };

      const [saving] = await sql`
        INSERT INTO savings (shg_id, member_id, month, year, amount, payment_mode, date)
        VALUES (${shgId}, ${memberId}, ${month}, ${year},
                ${amount}, ${args.payment_mode || 'cash'}, ${args.date || today})
    ...
    case 'get_loans': {
      ...
      if (args.member_id) {
        ...
        rows = await sql`
          ...
          WHERE l.shg_id = ${shgId} AND l.member_id = ${memberId}
    ...
    case 'create_loan': {
      ...
      const [loan] = await sql`
        ...
        VALUES (${shgId}, ${memberId}, ${principal}, ${rate}, ${months},
      ...
      for (let i = 1; i <= months; i++) {
        ...
        await sql`
          ...
          VALUES (${shgId}, ${loan.id}, ${memberId}, ${emi}, ${dueDate.toISOString().split('T')[0]}, 'pending')
        `;
      }
    ...
    case 'update_loan_status': {
      const loanId = toInt(args.loan_id);
      const [loan] = await sql`
        UPDATE loans SET status = ${args.status}, updated_at = now()
        WHERE id = ${loanId} AND shg_id = ${shgId}
        RETURNING *
      `;
    ...
    case 'get_repayments': {
      ...
      if (args.loan_id) {
        ...
        rows = await sql`
          ...
          WHERE r.shg_id = ${shgId} AND r.loan_id = ${loanId}
        ...
        rows = await sql`
          SELECT r.*, m.name AS member_name, l.loan_amount FROM repayments r
          JOIN members m ON m.id = r.member_id JOIN loans l ON l.id = r.loan_id
          WHERE r.shg_id = ${shgId} AND r.member_id = ${memberId}
          ORDER BY r.due_date ASC`;
      } else if (args.status) {
        rows = await sql`
          SELECT r.*, m.name AS member_name, l.loan_amount FROM repayments r
    ...
    case 'create_meeting': {
      const [meeting] = await sql`
        ...
        VALUES (${shgId}, ${args.date}, ${args.agenda}, ${args.venue || null},
...
router.post('/', requireShg, async (req, res) => {
  ...
  const langInstruction = language === 'en'
    ...
    : `Respond in ${LANGUAGE_NAMES[language] || language} only.`;

  // Short system prompt saves tokens on every request
  const systemPrompt = `You are Sakhi, a helpful AI assistant for ${shgInfo?.name || shgName || 'the SHG'}${location ? ` in ${location}` : ''}.
User: ${userName || 'leader'}. Today: ${new Date().toLocaleDateString('en-IN')}.

TOOL USAGE RULES — READ CAREFULLY:
You have two modes:

MODE 1 — Answer from your own knowledge (NO tools):
Use this for ANY question that is general, educational, or conversational. Examples:
- "What is a loan?" → just explain what a loan is. Do not fetch data.
- "What is interest?" → explain interest. Do not fetch data.
- "How is EMI calculated?" → explain EMI formula. Do not fetch data.
- "What is an SHG?" → explain Self Help Groups. Do not fetch data.
- "Hello / Hi / How are you" → just greet back. Do not fetch data.
- "What is BPL?" → explain BPL. Do not fetch data.
- Any general finance, banking, or knowledge question → answer directly. Do not fetch data.

MODE 2 — Use tools to fetch real SHG data:
ONLY use tools when the user asks about data specific to THEIR OWN SHG. Examples:
- "How many members do WE have?" → use get_members
- "Show OUR loans" → use get_loans
- "What are OUR total savings?" → use get_savings
- "Record ₹500 for Kavita" → use get_members then record_saving

The golden rule: if the question could be answered by any person with general knowledge, answer it yourself. Only fetch data if the answer requires looking at THIS SHG's database.

Additional rules:
- Always call get_members FIRST when a specific member is mentioned by name.
- Use ₹ for currency. Be warm, concise, and practical.
- ${langInstruction}`;

  // Trim to last 4 conversation pairs to save tokens
  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      // No tool calls → this is the final answer, send it back
      ...
      const toolResults = await Promise.all(
        assistantMsg.tool_calls.map(async (tc) => {
          ...
          return {
            role: 'tool',
            tool_call_id: tc.id,
            // Truncate large DB results to save tokens on the next round
    ...
    return res.json({ success: true, reply: 'I had trouble fetching data. Please rephrase your question.' });
    ...
    if (err.message.includes('Rate limit') || err.message.includes('rate limit')) {
      return res.status(429).json({
        success: false,
        error: 'The AI is a bit busy right now. Please wait 10–15 seconds and try again.',
      });
    }
